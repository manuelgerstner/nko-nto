...
export default function InvoiceDialog({ open, onClose, onSuccess, initialData, duplicateData }) {
  ...
  const validate = () => {
    const e = {};
    if (!form.number.trim()) e.number    = t('invoices.numberRequired');
    if (!form.issueDate)     e.issueDate = t('invoices.issueDateRequired');
    if (!form.currency)      e.currency  = t('invoices.currencyRequired');
    if (!form.status)        e.status    = t('invoices.statusRequired');
    if (!hasValidLines)      e.lines     = t('invoices.linesRequired');
    return e;
  };
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      ...
      <DialogContent dividers>
        <Stack spacing={2.5} sx={{ pt: 1 }}>
          ...
          <Stack direction="row" spacing={2}>
            <TextField
              label={t('invoices.invoiceNumber')} required fullWidth autoFocus
              value={form.number} onChange={setField('number')}
          ...
          <Stack direction="row" spacing={2}>
            <FormControl fullWidth>
              <InputLabel>{t('common.contact')}</InputLabel>
              <Select value={form.contactId} label={t('common.contact')} onChange={handleContactChange}>
      ...
      <DialogActions sx={{ px: 3, py: 2 }}>
        ...
        <Button variant="contained" onClick={handleSubmit} disabled={saving}>
          {saving ? t('common.saving') : isEdit ? t('common.saveChanges') : t('invoices.createInvoice')}
        </Button>
      </DialogActions>
    </Dialog>
  );
