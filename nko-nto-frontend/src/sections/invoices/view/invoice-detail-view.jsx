  ...
  return (
    <Stack spacing={2}>
      ...
      <Card sx={{ p: { xs: 3, sm: 5 }, position: 'relative' }}>
          ...
          <Box sx={{ textAlign: { sm: 'right' }, mt: { sm: 5 } }}>
            <Stack spacing={0.5} alignItems={{ sm: 'flex-end' }}>
              <Stack direction="row" spacing={2} justifyContent={{ sm: 'flex-end' }}>
                ...
                <Typography variant="body2" fontWeight={500}>{formatDate(invoice.issueDate)}</Typography>
              </Stack>
              <Stack direction="row" spacing={2} justifyContent={{ sm: 'flex-end' }}>
                <Typography variant="body2" color="text.secondary">{t('common.dueDate')}:</Typography>
                <Typography variant="body2" fontWeight={500}>{formatDate(invoice.dueDate)}</Typography>
              </Stack>
              <Stack direction="row" spacing={2} justifyContent={{ sm: 'flex-end' }}>
        ...
        {/* Bill To */}
        <Box sx={{ mb: 4 }}>
          <AddressBlock contact={invoice.contact} label={t('invoices.billTo')} />
        </Box>
      ...
      <InvoiceDialog
        ...
        onSuccess={() => { setDuplicateOpen(false); navigate('/invoices'); }}
        duplicateData={invoice}
      />
    </Stack>
  );
}
